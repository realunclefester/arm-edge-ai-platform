...
if not DATABASE_URL:
    ...
    if password:
        DATABASE_URL = (
            f"postgresql://ai_platform_user:{password}@localhost:5432/ai_platform_db"
        )
    else:
        raise ValueError(
            "DATABASE_URL or POSTGRES_PASSWORD environment variable is required"
        )
TABLE_NAME = "ai_platform_memory"  # pgvector table name
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                f"{EMBEDDINGS_URL}/embed", json={"texts": [text]}, timeout=30.0
            )
...
async def call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    if name == "store_memory":
        ...
        if not embedding:
            return [
                TextContent(type="text", text="Error: Failed to generate embedding")
            ]
        ...
        try:
            ...
            )  # microsecond timestamp as string
        ...
        if not embedding:
            return [
                TextContent(
                    type="text", text="Error: Failed to generate query embedding"
                )
            ]
