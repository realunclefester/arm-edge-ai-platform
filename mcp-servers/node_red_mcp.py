...
async def search_nodes(search_term: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for node in flows:
            ...
            if (
                search_lower in node.get("name", "").lower()
                or search_lower in node.get("type", "").lower()
                or search_lower in node.get("info", "").lower()
                or search_lower in str(node.get("func", "")).lower()
                or search_lower in str(node.get("url", "")).lower()
                or search_lower in str(node.get("query", "")).lower()
            ):
    ...
    try:
        ...
        for node in flows:
            if node.get("id") == node_id:
                ...
                if node.get("z"):
                    for flow in flows:
                        if flow.get("type") == "tab" and flow.get("id") == node.get(
                            "z"
                        ):
    ...
    try:
        ...
        if "error" not in test_result:
            ...
            if "error" not in stats:
                logger.info(
                    f"Node-RED: {stats.get('flow_count', 0)} flows, {stats.get('node_count', 0)} nodes"
                )
        else:
