...
def get_system_status() -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            temperature = (
                temp_result.stdout.strip().replace("temp=", "").replace("'", "")
            )
        except:
            temperature = "N/A"
...
def watchdog_check():
    ...
    try:
        ...
        for service_name, is_healthy in services.items():
            if not is_healthy:
                alert_to_stderr(
                    f"Service {service_name} is down or unreachable", "ERROR"
                )
...
async def call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    if name == "get_system_status":
        ...
        return [TextContent(type="text", text=json.dumps(disk_info, indent=2))]
        ...
        return [TextContent(type="text", text=json.dumps(conn_info, indent=2))]
                ...
                text=json.dumps(
                    {
                        ...
                        "message": (
                            "Watchdog started"
                            if success
                            else "Watchdog already running or disabled"
                        ),
                    },
                    indent=2,
                ),
